package com.ssafy.curtaincall.video.controller;
...
import com.ssafy.curtaincall.video.service.VideoService;
...
public class VideoController {
	...
	@Autowired
	@Qualifier("videoServiceImpl") // 서비스 빈이 두개 찾아진다는 오류 때문에 추가했어용
	 ...
	 */
	@GetMapping("/{id}")
	public ResponseEntity<Video> getMusical(@PathVariable int id) {
		Video video = service.getVideo(id);
		if (video == null) return ResponseEntity.noContent().build();
		else return ResponseEntity.ok(video);
	}
	/* 1-3. 조회수 많은 순 영상 조회
	 *
	 * 메서드 : GET
	 ...
	 * 파라미터
	 *   - 없음
	 * 리턴 : List<Video> 조회수 많은 순 전체 영상 목록
	 */
	@GetMapping("/views")
	public ResponseEntity<List<Video>> getVideosByViews() {
	    List<Video> videos = service.getVideoByViews();
